...
*EXPERIMENTAL* - A powershell script to clear the Adobe Acrobat cache. Other utilities like CCleaner or Glary's Utilities should be able to do that, but I don't much like installing CC on people's computers and GU doesn't reliably clear the cache. It's basically the same as the batch script, but calculates a total size in megabytes of the cache to be cleared. Run with the following batch script to avoid annoying PS permission prompts. If there are no files in one of the locations it clears there may be an error that it can't sum without files present, but this is hardly a bug when you should be using this to clear cache and not use it immediately again after, that would be silly.
